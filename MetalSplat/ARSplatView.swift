...
//        self.splatCloud = try! SplatCloud(model: model, renderDestination: metalKitView, index: 0)
//        
//        
...
//        drag.setup(view: metalKitView, camera: camera, object: self.splatCloud! )
        ...
        let numberOfSplatClouds = currentFrameNum
        
        let videoProcessor = VideoProcessor()
        let videosFrames = videoProcessor.processVideos(groupIndex: 0, dataIndex: 0)
        let minmax = loadAndExtractData(from: 0, to: currentFrameNum, dataIndex: 0)

        for index in 0..<numberOfSplatClouds {
            do {
                let oneFrame = extractFirstFrames(from: videosFrames, at: index)
                let splatCloud = try SplatCloud(model: model, renderDestination: metalKitView, groupFrame: videosFrames, minmax: minmax[index], frameIndex: index, dataIndex: 0)
                splatCloud?.orientation = model.initialOrientation
                splatCloud?.scale = .init(repeating: model.initialScale)
    ...
    @objc private func handleTapGesture() {
        ...
        self.splatClouds[0]?.isDragging = true
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            self.splatClouds[0]?.isDragging = false
        }
    ...
    override func draw(_ view: MTKView, _ commandBuffer: MTLCommandBuffer) {
        ...
        renderTime += 1
        if renderTime == 4 * currentFrameNum {
            renderTime = 0
            let newSplatCloud = self.splatClouds[renderTime]!
            scene.remove(self.splatClouds[currentFrameNum - 1]!)
            scene.add(newSplatCloud)
        }
        else if renderTime % 4 == 0 {
            let newSplatCloud = self.splatClouds[renderTime / 4]!
            scene.remove(self.splatClouds[renderTime / 4 - 1]!)
            scene.add(newSplatCloud)
        }
